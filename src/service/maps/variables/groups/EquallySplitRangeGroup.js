...
export default class EquallySplitRangeGroup extends RangeGroup {
   ...
   */
  _createRanges() {
    if (!this._canEdit) return this._getDefaultRanges();
    ...
    else if (isEmpty(this._ranges)) result = this._getDefaultRanges();
    return result;
  }
  ...
  _getDefaultRanges() {
    ...
    const start = min + ((max - min) * this._rangeStart) / 100;
    const end = min + ((max - min) * this._rangeEnd) / 100;
    const isStartShift = start > min;
    const isEndShift = end < max;
    const intervalSize = Math.abs(end - start) / count;
  ...
  update(data) {
    if (data.min || data.min === 0 || data.max) {
      if (data.min || data.min === 0) this._rangeStart = data.min;
      if (data.max) this._rangeEnd = data.max;
      this._ranges = this._createRanges();
    }
    if (data.ranges) {
