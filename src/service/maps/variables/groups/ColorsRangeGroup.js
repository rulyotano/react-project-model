...
export default class ColorsRangeGroup extends RangeGroup {   
    constructor(id, name, rangeFn, items, valueFn, displayFn, size = 8, colors=["white", "red"], rangeSuffix = "") {
    ...
    */
    ...
    _getRanges(){
        ...
        for (let val=min, clr = 0, i = 0; i < n; i++, val+=valueInc, clr+=colorsInc) {            
            result.push(new Range(i, colorMap(clr), null, null, val, val + valueInc, this._rangeSuffix, this._displayFn));            
        }
        return result;
    }
    ...
    update(values){
        ...
        if (!values.color && values.size === undefined)
            return;
