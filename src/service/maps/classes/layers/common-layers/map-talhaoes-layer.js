...
export default class MapTalhaoesLayer extends MapCommonLayer {
  ...
  static talhaoDefaultString(feature, customTxt = ""){
    ...
    const fazenda = props.cdFazenda;
    const setor = props.cdZona;
    const {cdTalhao} = props;
    const nameFazenda = props.nomeFazenda || '-';
    const areaTotal = round(props.area, 2);
    const {descTalhao} = props;

    const descTalhaoStr = descTalhao && descTalhao !== 'null' ? `<p><h5>Desc. ${t('maps.Field')}:<small> ${descTalhao}</small></h5></p>` : '';
