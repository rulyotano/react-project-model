...
import closeFieldMapUrl, { PreloadKey as MAP_KEY } from "../../map/routesNames";
import closeFieldProcessUrl, {
  PreloadKey as PROCESS_KEY
} from "../../process/routesNames";
...
const processMapData = data => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    item.dtHrLocalInicial = moment(
      item.dtHrLocalInicial,
      SERVER_DATA_FORMAT
    ).valueOf();
    item.dtHrLocalFinal = moment(
      item.dtHrLocalFinal,
      SERVER_DATA_FORMAT
    ).valueOf();
  }
...
const processProcessData = data => {
  ...
  return chain(data)
    ...
    .map((items, key) => ({
      ...
      machineArea: chain(items)
        .sumBy(it => it.machineArea)
        .round(2)
        .value(),
      date: groupRowValues(items, it =>
        fromServerArray(it.date).format(DATE_FORMAT)
      ),
      state: "Active"
    }))
    .value();
...
export const load = (data, source) => (dispatch, getState) => {
  ...
  if (state.app.closeField.load.loading) return;
  ...
  if (dateRange) {
    if (dateRange.initialDateTime) {
      params.dataHoraInicial = moment(dateRange.initialDateTime).format(
        DATE_SERVER_FORMAT
      );
    }
    if (dateRange.finalDateTime) {
      params.dataHoraFinal = moment(dateRange.finalDateTime).format(
        DATE_SERVER_FORMAT
      );
    }
  }
  ...
  httpService
    ...
    .then(
      response => {
        ...
        dispatch({
          type: MAP_LOAD,
          data: processMapData(response.listaDeRetorno)
        });
        // set loaded filters
