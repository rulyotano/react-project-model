...
import {
  ...
  REMOVE_PAST_BOTTOM_NOTIFICATION
} from "../actions/bottomNotificationActions.types";
...
export default (state = initialState, action) => {
  ...
  case REMOVE_DIALOG: {
    ...
    return {
      ...state,
      dialogs: filter(state.dialogs, d => d.Id !== dialogId)
    };
  }
  case ADD_BOTTOM_NOTIFICATION: {
    const lastNotificationTime = moment(
      get(last(state.bottomNotifications), "TimeToClose")
    );
    return {
      ...state,
      bottomNotifications: [
  ...
  case REMOVE_PAST_BOTTOM_NOTIFICATION:
    if (
      isEmpty(state.bottomNotifications) ||
        every(state.bottomNotifications, bn => !bn.shouldClose())
    )
      return state;
    return {
      ...state,
      bottomNotifications: filter(
        state.bottomNotifications,
        bn => !bn.shouldClose()
      )
    };
