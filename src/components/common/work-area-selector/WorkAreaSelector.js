...
import Loading from "../_LoadingComponent";
import {loadMapGeoJson} from "../map/_duck/actions";
...
class WorkAreaSelectorComponent extends PureComponent {
    ...
    handleOnBlur() {
      ...
      const matches = filter(mapGeoJson.features, f => f.properties.cdTalhao === field
    ...
    render() {
      ...
      const { farm = "", sector = "", field = "" } = value && !isString(value) ? value : {};
      t = this.context.t;
...
const mapStateToProps = (state, props) => {
  const getDefaultFarm = () => {
    const map = state.map.mapMappedGeoJson;
    if (!map)
      return null;
